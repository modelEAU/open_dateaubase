...
These tests verify:
- Unknown channel_kind / parameter_name / unit_name -> 422 before any DB write
...
class TestValidationErrors:
    def test_unknown_channel_kind_returns_422(self, client, mock_conn):
        payload = {**_VALID_PAYLOAD, "channel_kind": "not_a_type"}
        with (
            ...
            patch(f"{_REPO}.find_signal_interface_by_das_and_name") as mock_si,
...
class TestHappyPath:
    ...
    def test_no_warnings_when_das_and_port_exist(self, client, mock_conn):
        with _patch_all_resolved(das_created=False, si_created=False):
            resp = client.post("/api/v1/ingest/sensor", json=_VALID_PAYLOAD)
    ...
    def test_warnings_returned_when_das_auto_created(self, client, mock_conn):
        with _patch_all_resolved(das_created=True, si_created=False):
            resp = client.post("/api/v1/ingest/sensor", json=_VALID_PAYLOAD)
    ...
    def test_warnings_returned_when_signal_interface_auto_created(
        self, client, mock_conn
    ):
        with _patch_all_resolved(das_created=False, si_created=True):
            resp = client.post("/api/v1/ingest/sensor", json=_VALID_PAYLOAD)
    ...
    def test_two_warnings_when_both_auto_created(self, client, mock_conn):
        with _patch_all_resolved(das_created=True, si_created=True):
            resp = client.post("/api/v1/ingest/sensor", json=_VALID_PAYLOAD)
...
class TestNormalisation:
    ...
    def test_dirty_input_passed_to_repo_functions(self, client, mock_conn):
        ...
        with (
            patch(f"{_REPO}.find_channel_kind_by_name", return_value=1) as mock_role,
            ...
            patch(f"{_REPO}.find_signal_interface_by_das_and_name", return_value=20),
        ...
        mock_role.assert_called_once_with(mock_conn[0], "  VALUE  ")
