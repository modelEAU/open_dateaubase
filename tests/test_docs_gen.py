...
class TestParsePartsTable:
    ...
    def test_parse_identifies_foreign_keys(self, sample_csv_file):
        ...
        fk_field = next(
            f
            for f in data["tables"]["TestTable"]["fields"]
    ...
    def test_parse_links_value_sets_to_fields(self, sample_csv_file):
        ...
        status_field = next(
            f for f in data["tables"]["TestTable"]["fields"] if f["part_id"] == "Status"
        )
...
class TestGenerateSQLSchemas:
    ...
    def test_generates_sql_file_with_timestamp_and_version(
        self, sample_csv_file, tmp_path
    ):
...
class TestOnPreBuild:
    ...
    def mock_config(self, tmp_path, sample_csv_data):
        ...
        # Create dictionary JSON (convert from sample CSV data)
        json_file = src_dir / "dictionary.json"

        # Convert sample CSV data to basic JSON structure for testing
        import json
        import csv
        from io import StringIO

        # Parse CSV data
        csv_reader = csv.DictReader(StringIO(sample_csv_data))
...
def test_reads_json_from_project_root(self, mock_config):
    """Test that on_pre_build correctly locates dictionary.json in src directory."""
    config, project_root, docs_dir, reference_dir, sql_dir = mock_config

    # JSON was created in src directory by fixture
