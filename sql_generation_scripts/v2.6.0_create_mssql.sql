-- Baseline CREATE script for schema v2.6.0
-- Platform: mssql
...
CREATE TABLE [dbo].[ProcessUnitKind] (
    ...
    CONSTRAINT [CK_ProcessUnitKind_Category] CHECK ([Category] IS NULL OR [Category] IN ('Structural', 'Treatment', 'Conveyance'))
);
...
CREATE TABLE [dbo].[AnalysisSeries] (
    ...
    CONSTRAINT [UQ_AnalysisSeries_Identity] UNIQUE ([Parameter_ID], [SamplingPoint_ID], [ValueKind_ID], [Laboratory_ID])
);
...
CREATE TABLE [dbo].[LabPanel] (
    ...
    [DefaultSampleKind_ID] INT,
    [DefaultSampleMaterialKind_ID] INT,
...
CREATE TABLE [dbo].[Sample] (
    ...
    CONSTRAINT [UQ_Sample_Identity] UNIQUE ([SamplingPoint_ID], [SampleDateTimeStart], [SampleKind_ID], [Replicate], [ParentSample_ID])
);
...
ALTER TABLE [dbo].[AnalysisSeries] ADD CONSTRAINT [FK_AnalysisSeries_Laboratory_ID] FOREIGN KEY ([Laboratory_ID]) REFERENCES [dbo].[Laboratory] ([Laboratory_ID]);
...
ALTER TABLE [dbo].[ProcessUnit] ADD CONSTRAINT [FK_ProcessUnit_TreatmentStage_ID] FOREIGN KEY ([TreatmentStage_ID]) REFERENCES [dbo].[TreatmentStage] ([TreatmentStage_ID]);
...
VALUES (N'2.6.0', N'Process unit vocabulary rebuilt on the device and stage axes: ProcessUnitKind gains a Category (Structural / Treatment / Conveyance) and the named treatment nouns it lacked, TreatmentStage arrives as its own vocabulary referenced by ProcessUnit so a clarifier is primary or secondary by role rather than by kind. Also splits OperationKind''s Interpolation from Reconstruction, and adds Person.IsActive.');
